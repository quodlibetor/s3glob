...
mod tests {
    ...
    fn test_find_prefixes_alternation_no_any() -> Result<()> {
        ...
        engine.assert_calls(&[("src/foo/baz", "/"), ("src/bar/baz", "/")]);
        Ok(())
    }
    ...
    fn test_find_prefixes_recursive() -> Result<()> {
        ...
        let expected: &[(&str, &str)] = &[("src/", "/")];
        engine.assert_calls(expected);
        Ok(())
    }
